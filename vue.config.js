...
const webpackerConfig = yaml.safeLoad(
  fs.readFileSync(path.resolve(__dirname, "config/webpacker.yml"), "utf8")
);
...
const outputDir = `${webpackerConfig.production.public_root_path}/${publicPath}`;
...
const devServer = Object.assign(
  {},
  ...Object.entries(omit(devServerConfig, ["hmr", "pretty"])).map(
    ([field, value]) => {
      return {
